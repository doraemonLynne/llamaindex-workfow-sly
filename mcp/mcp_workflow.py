...
# 测试 AntV Chart 服务器连接的独立函数
async def test_chart_server_connection(chart_server_url: str = "http://127.0.0.1:3000/sse"):
    """Test AntV chart server connection independently"""
    try:
        print(f"Testing AntV chart server connection to: {chart_server_url}")
        
        # Create MCP client
        mcp_client = BasicMCPClient(chart_server_url)
        ...
        print(f"Connection successful! Available chart tools: {[tool.metadata.name for tool in tools]}")
        
        # Test a chart generation if available
        for tool in tools:
            if tool.metadata.name == "generate_line_chart":
        ...
        print(f"Chart server connection test failed: {str(e)}")
        return False


# Usage example
async def run_chart_workflow_example():
    """Run chart generation workflow example"""
    try:
        # 首先测试 AntV Chart 服务器连接
        print("=== Testing AntV Chart Server Connection ===")
        connection_ok = await test_chart_server_connection()
        
        if not connection_ok:
            print("AntV chart server connection failed. Please start the mcp-server-chart first.")
            print("Install: npm install -g @antv/mcp-server-chart")
            print("Run: mcp-server-chart --transport sse")
            return {"error": "AntV chart server not available"}
        
        print("\n=== Running Chart Generation Workflow ===")
        ...
        workflow = ChartMCPWorkflow(
            llm=llm,
            mcp_server_url="http://127.0.0.1:3000/sse",  # AntV chart server
            allowed_tools=["generate_line_chart", "generate_bar_chart", "generate_pie_chart"]  # Allow specific chart types
        )
        
        # Run workflow with chart generation request
        result = await workflow.run(
            user_msg="Please generate a line chart showing sales trends over the past 5 months",
            chat_history=[]
        )
        
        print("\n=== Chart Generation Result ===")
        print(f"Response: {result['response']}")
        if 'available_chart_tools' in result:
            print(f"Available Chart Tools: {result['available_chart_tools']}")
        if 'tool_calls' in result and result['tool_calls']:
            print(f"Tool Calls: {result['tool_calls']}")
        if 'tool_results' in result and result['tool_results']:
            print(f"Chart Results: {result['tool_results']}")
        if 'chart_capabilities' in result:
            print(f"Chart Capabilities: {result['chart_capabilities']}")
        if 'error' in result:
        ...
        print(f"Error running chart workflow: {str(e)}")
        return {"error": str(e)}


if __name__ == "__main__":
    # Run chart generation example
    asyncio.run(run_chart_workflow_example())